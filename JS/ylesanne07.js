...
const t = [
  ...
  const m = "Jaanuar, Veebruar, Märts, Aprill, Mai, Juuni, Juuli, August, September, Oktoober, November, Detsember".split(", ");
  ...
  for (let i = 0; i < t.length; i++) {
    const avgTemp = t[i].reduce((sum, temp) => sum + temp, 0) / t[i].length;
    ...
    console.log(`Kuu ${m[i]} keskmine temperatuur: ${avgTemp.toFixed(2)} kraadi.`);
  }
  
  console.log(`Kõrgeim keskmine temperatuur oli kuus "${maxMonth}" ja oli ${maxTemp.toFixed(2)} kraadi.`);
  console.log(`Madalaim keskmine temperatuur oli kuus "${minMonth}" ja oli ${minTemp.toFixed(2)} kraadi.`);
